/**
 * Jail Intakes dry-run plan.
 *
 * Reads the jail source scan JSONL and generates exact API payloads
 * for every proposed update. Outputs a plan file for human review.
 *
 * Zero writes to Monday.com.
 *
 * Usage: bun src/jail-dry-run-plan.ts
 */
...
// ---------------------------------------------------------------------------
// Types (matching jail-source-scan output)
// ---------------------------------------------------------------------------
...
function main() {
  ...
  console.log(`Reading jail source scan report: ${reportPath}\n`);
  ...
  const summaryPath = `reports/jail-dry-run-summary-${new Date().toISOString().slice(0, 10)}.txt`;
  ...
  summaryLines.push("JAIL INTAKES DRY-RUN PLAN SUMMARY");
  summaryLines.push("=".repeat(70));
  summaryLines.push("");
  ...
  for (const p of samples) {
    ...
    for (const u of p.updates) {
      ...
      summaryLines.push(`    Source: Jail Intakes, item ${u.sourceAppointmentId} (consult: ${u.sourceConsultDate ?? "no date"})`);
    }
