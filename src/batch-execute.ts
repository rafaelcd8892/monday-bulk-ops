/**
 * Phase 4/5: Execute updates from the dry-run plan.
 *
 * Safety:
 *   - Reads the plan JSONL
 ...
 *
 * Usage:
 *   bun src/batch-execute.ts                        # dry-run (default), no writes
 *   bun src/batch-execute.ts --limit 10             # dry-run, first 10 profiles
 *   bun src/batch-execute.ts --limit 10 --confirm   # LIVE, first 10 profiles
 *   bun src/batch-execute.ts --confirm              # LIVE, all profiles
 */
...
function initLog() {
  ...
  logPath = `logs/batch-${mode}-${ts}.jsonl`;
}
...
async function main() {
  ...
  const planPath = "reports/dry-run-plan-2026-03-19.jsonl";
  ...
  // Summary
  console.log(`\n${"=".repeat(60)}`);
  console.log(`EXECUTION SUMMARY (${confirm ? "LIVE" : "DRY-RUN"})`);
